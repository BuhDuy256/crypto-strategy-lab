...
| Last verified commit | `e777b1c`, the committed NEWS-04 implementation on `v5-news-and-sentiment`. `c46c2e1`, `93958a9`, and `6f387d6` are the committed NEWS-03, NEWS-02, and NEWS-01 slices. |
| Next allowed action | `NEWS-05` is `DONE` (all 6 acceptance criteria proved by TDD; see JOURNAL.md); its full diff is still uncommitted, since the owner deferred code-review/full-suite/commit for this slice this session. `NEWS-07` is now `READY`. The NEWS-01..NEWS-03 slice-diff review remains owner-deferred; `UI-07` remains dependency-blocked. |
| Last verified on | 2026-08-31 (NEWS-05 implementation complete at the seam level; V4 certification remains unchanged). |
...
| V5 slices | 7 required plus 1 optional (`DONE` 5, `READY` 1, `IN_PROGRESS` 0, `BLOCKED` 0, `TODO` 2) — `NEWS-01` through `NEWS-04` are committed (`6f387d6`, `93958a9`, `c46c2e1`, `e777b1c`); the owner deferred the NEWS-01..NEWS-03 combined slice-diff review; `NEWS-05` is done in the working tree but uncommitted; `NEWS-07` is `READY`. |
...
| NEWS-05 | REQ | S | Sentiment feature query and degradation policy | **DONE** | NEWS-04 | All 6 acceptance criteria proved by 14 TDD cycles: windowed signed-mean query with window/item-count/freshness/quality; missing and stale block/degrade/substitute all visible in the response; technical-only descriptors make zero News calls; a supplied multi-window usage manifest is recorded as `newsInput`/`sentimentModel` in the provenance checklist. Typecheck, ESLint, and 37 targeted tests pass. Working tree is uncommitted; the owner deferred code-review/full-suite/commit for this slice this session. See JOURNAL.md. | [05](05-news-and-sentiment.md) |
| NEWS-07 | REQ | S | News list, health, and sentiment query surface | **READY** | NEWS-05 | | [05](05-news-and-sentiment.md) |

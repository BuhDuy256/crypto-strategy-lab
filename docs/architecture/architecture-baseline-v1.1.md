...
- `BacktestResult`: one logical result per idempotency key, with metrics, required provenance/build references, and either directly stored trades or an immutable trade-data reference plus content hash.
...
All correctness-relevant publication uses an outbox and BullMQ. An outbox entry may be marked delivered only after the dispatcher receives successful BullMQ enqueue acknowledgement for durable job creation; a stable event-derived job ID plus consumer idempotency makes retry after an ambiguous dispatcher failure safe. Redis used for BullMQ must have persistence configured and verified. Consumers use event-ID deduplication/inbox state and aggregate-version checks. Duplicate delivery must be a no-op. Ordering assumptions must be explicit per aggregate/key. Redis Pub/Sub acknowledgement is never evidence of durable delivery.
...
6. backtest engine/build, Node.js runtime, dependency-lock identity/hash, application and worker build/commit, and deterministic runtime configuration;
...
- Node.js + TypeScript for the core backend, domain/application realization, and initial workers.
...
2. Cross-module access occurs only through exported application/domain ports.
3. A module must not import another module's repository implementation, infrastructure adapter, ORM model/repository, or private provider.
4. NestJS decorators and types must not leak into framework-independent domain contracts where avoidable.
5. Dependency injection is used at composition/application boundaries, not as a service locator inside domain logic.
6. Shared/common modules contain only genuinely technical primitives and must not become a business-domain dumping ground.
7. Backtest workers use the same versioned domain/application contracts but execute outside the interactive API/WebSocket process.
8. A NestJS module is implementation support for a frozen logical boundary; creating one does not by itself prove a valid bounded context.

These invariants are implementation constraints to be enforced with dependency/import architecture tests. They do not couple the Strategy, provider, experiment, or sentiment domain contracts to NestJS.

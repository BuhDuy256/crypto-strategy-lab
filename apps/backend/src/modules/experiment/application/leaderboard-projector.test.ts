...
const specifications: ProjectionSpecifications = {
  get: (specId: string): Promise<ExperimentSpecification> => {
    if (specId !== LEADERBOARD) return Promise.reject(new Error(`unknown spec ${specId}`));
...
class FakeStore implements LeaderboardProjectionStore {
  ...
  async withLeaderboard<T>(
    ...
    const scope: LeaderboardWriteScope = {
      ...
      appliedVersion: (contentHash) => Promise.resolve(this.applied.get(contentHash)),
      recordApplied: (contentHash, aggregateVersion) => {
        this.applied.set(contentHash, aggregateVersion);
        return Promise.resolve();
      },
      resetAppliedVersions: (versions) => {
        this.applied = new Map(versions.map((v) => [v.contentHash, v.aggregateVersion]));
        return Promise.resolve();
      }
    };
    return run(scope);
...
function candidate(
  ...
  store.memberships.set(runId, {
    ...
    contentHash,
    derivedSpecId: `spec-${name}`
  });
  return { resultId: `result-${name}`, runId, aggregateVersion, metrics: values };
}
...
describe("LeaderboardProjector", () => {
  ...
  it("rebuilds the same Top-K set the incremental applies produced", async () => {
    ...
    store.completed = inputs.map((input) => ({
      contentHash: store.memberships.get(input.runId)!.contentHash,
      resultId: input.resultId,
      runId: input.runId,
      derivedSpecId: store.memberships.get(input.runId)!.derivedSpecId,
